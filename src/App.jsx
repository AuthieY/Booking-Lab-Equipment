...
// --- Firebase Init ---
// ⚠️⚠️⚠️ 请记得把下面这行换成你自己的 Firebase 配置！
...
const GateScreen = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isCreating) {
        if (labData) throw new Error("该实验室名称已被注册");
        if (!newAdminPass || !newMemberPass) throw new Error("请完整填写密码");
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6 animate-fade-in">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 transition-all">
        ...
        <h1 className="text-2xl font-bold text-center text-slate-800 mb-2">{isCreating ? '创建实验室' : role === 'ADMIN' ? '管理员登录' : '进入实验室'}</h1>
        {!isCreating && (<div className="flex bg-slate-100 p-1 rounded-xl mb-6"><button onClick={()=>setRole('MEMBER')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition ${role==='MEMBER'?'bg-white shadow text-blue-600':'text-slate-400'}`}>普通成员</button><button onClick={()=>setRole('ADMIN')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition ${role==='ADMIN'?'bg-white shadow text-slate-800':'text-slate-400'}`}>管理员 (Admin)</button></div>)}
        <form onSubmit={handleSubmit} className="space-y-4">
          <div><label className="text-xs font-bold text-slate-400 uppercase ml-1">实验室名称</label><input type="text" value={labName} onChange={(e) => setLabName(e.target.value)} placeholder={isCreating ? "如: BioLab-X" : "输入名称"} className="w-full p-3 rounded-xl border-2 border-slate-100 focus:border-blue-500 focus:outline-none transition" /></div>
          {isCreating ? (<><div className="p-4 bg-slate-50 rounded-xl border border-slate-100"><label className="text-xs font-bold text-slate-500 uppercase ml-1">设置管理员密码</label><input type="password" value={newAdminPass} onChange={(e) => setNewAdminPass(e.target.value)} className="w-full mt-2 p-3 rounded-lg border border-slate-200 focus:border-slate-800 focus:outline-none transition text-sm" /></div><div className="p-4 bg-blue-50 rounded-xl border border-blue-100"><label className="text-xs font-bold text-blue-600 uppercase ml-1">设置成员通行码</label><input type="password" value={newMemberPass} onChange={(e) => setNewMemberPass(e.target.value)} className="w-full mt-2 p-3 rounded-lg border border-blue-200 focus:border-blue-500 focus:outline-none transition text-sm" /></div></>) : (<div><label className="text-xs font-bold text-slate-400 uppercase ml-1">{role === 'ADMIN' ? '管理员密码' : '成员通行码'}</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className={`w-full p-3 rounded-xl border-2 border-slate-100 focus:outline-none transition ${role==='ADMIN' ? 'focus:border-slate-800' : 'focus:border-blue-500'}`} /></div>)}
          ...
          <button type="submit" disabled={loading} className={`w-full text-white font-bold py-4 rounded-xl transition shadow-lg mt-4 flex items-center justify-center gap-2 ${isCreating ? 'bg-indigo-600 hover:bg-indigo-700' : role==='ADMIN'?'bg-slate-800 hover:bg-slate-900':'bg-blue-600 hover:bg-blue-700'}`}>{loading ? <Loader2 className="animate-spin w-5 h-5" /> : isCreating ? "立即创建" : "登录系统"}</button>
        </form>
        <div className="mt-6 text-center border-t border-slate-100 pt-4"><button onClick={()=>{setIsCreating(!isCreating); setError(''); setPassword('');}} className="text-sm font-bold text-slate-500 hover:text-blue-600 underline">{isCreating ? '已有实验室? 直接登录' : '创建新实验室'}</button></div>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans">
        <header className="bg-slate-800 text-white px-6 py-4 flex justify-between items-center sticky top-0 z-50 shadow-md">
            <div className="flex items-center gap-3"><div className="bg-slate-700 p-2 rounded-lg"><ShieldCheck className="w-6 h-6 text-yellow-400"/></div><div><h1 className="font-bold text-lg leading-tight">{labName} 管理后台</h1><p className="text-xs text-slate-400">Admin Dashboard</p></div></div>
            <button onClick={onLogout} className="text-slate-300 hover:text-white flex items-center gap-2 text-sm"><LogOut className="w-4 h-4"/> 退出</button>
        </header>
        <div className="p-6 max-w-5xl mx-auto">
            <div className="flex gap-4 mb-6">
                <button onClick={()=>setActiveTab('INSTRUMENTS')} className={`flex-1 p-4 rounded-2xl flex items-center justify-center gap-3 transition font-bold ${activeTab==='INSTRUMENTS'?'bg-white shadow-lg text-slate-800':'bg-slate-200 text-slate-500 hover:bg-slate-300'}`}><Settings className="w-5 h-5"/> 仪器配置</button>
                <button onClick={()=>setActiveTab('LOGS')} className={`flex-1 p-4 rounded-2xl flex items-center justify-center gap-3 transition font-bold ${activeTab==='LOGS'?'bg-white shadow-lg text-slate-800':'bg-slate-200 text-slate-500 hover:bg-slate-300'}`}><History className="w-5 h-5"/> 审计日志</button>
            </div>
            {activeTab === 'INSTRUMENTS' ? (
                <div className="bg-white rounded-3xl p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold text-slate-800">仪器列表 ({instruments.length})</h2><button onClick={()=>setShowAddModal(true)} className="bg-slate-900 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-slate-700 flex items-center gap-2"><Plus className="w-4 h-4"/> 添加设备</button></div>
                    <div className="space-y-3">{instruments.map(inst => (<div key={inst.id} className="flex items-center justify-between p-4 border border-slate-100 rounded-xl hover:border-slate-300 transition bg-slate-50"><div className="flex items-center gap-4"><div className={`w-12 h-12 rounded-lg flex items-center justify-center ${getColorStyle(inst.color).bg} ${getColorStyle(inst.color).text} font-bold text-xl`}>{inst.name[0]}</div><div><div className="font-bold text-slate-800">{inst.name}</div><div className="text-xs text-slate-500 flex items-center gap-1"><MapPin className="w-3 h-3"/> {inst.location || '未设置位置'}</div><div className="text-[10px] text-slate-400 mt-1 flex flex-wrap gap-1">{inst.subOptions?.map(o=><span key={o} className="bg-slate-100 px-1 rounded">{o}</span>)}</div></div></div><button onClick={()=>handleDeleteInstrument(inst.id, inst.name)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition"><Trash2 className="w-5 h-5"/></button></div>))}</div>
                </div>
            ) : (
                <div className="bg-white rounded-3xl p-6 shadow-sm"><h2 className="text-xl font-bold text-slate-800 mb-6">操作日志</h2><div className="overflow-hidden rounded-xl border border-slate-100"><table className="w-full text-sm text-left"><thead className="bg-slate-50 text-slate-500 font-medium"><tr><th className="p-4">时间</th><th className="p-4">用户</th><th className="p-4">动作</th><th className="p-4">详情</th></tr></thead><tbody className="divide-y divide-slate-100">{logs.map(log => (<tr key={log.id} className="hover:bg-slate-50"><td className="p-4 text-slate-400 font-mono text-xs">{formatTime(log.timestamp)}</td><td className="p-4 font-bold text-slate-700">{log.userName}</td><td className="p-4"><span className={`px-2 py-1 rounded text-xs font-bold ${log.action.includes('DEL') || log.action.includes('CANCEL') ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'}`}>{log.action}</span></td><td className="p-4 text-slate-600">{log.message}</td></tr>))}</tbody></table></div></div>
            )}
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
        <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
            <h3 className="text-lg font-bold mb-4 text-slate-800">添加新设备</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div><label className="text-xs font-bold text-slate-400 uppercase ml-1">设备名称 *</label><input autoFocus type="text" value={name} onChange={e=>setName(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-slate-800 outline-none"/></div>
                <div><label className="text-xs font-bold text-slate-400 uppercase ml-1">放置位置</label><input type="text" value={location} onChange={e=>setLocation(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-slate-800 outline-none"/></div>
                <div>
                    <label className="text-xs font-bold text-slate-400 uppercase ml-1 flex items-center gap-1"><ListPlus className="w-3 h-3"/> 配套附件/进样口 (选填)</label>
                    <textarea value={subOptionsStr} onChange={e=>setSubOptionsStr(e.target.value)} placeholder="如: 干法进样, 湿法进样 (用逗号分隔)" className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-slate-800 outline-none text-sm h-20 resize-none"/>
                    <p className="text-[10px] text-slate-400 mt-1 ml-1">预约时用户必须从中选择一个</p>
                </div>
                <div><label className="text-xs font-bold text-slate-400 uppercase ml-1">图片链接</label><input type="text" value={image} onChange={e=>setImage(e.target.value)} className="w-full border-2 border-slate-100 p-3 rounded-xl focus:border-slate-800 outline-none text-xs"/></div>
                ...
                <div className="flex gap-3 mt-6"><button type="button" onClick={onClose} className="flex-1 py-3 text-slate-500 font-bold bg-slate-50 rounded-xl">取消</button><button type="submit" className="flex-1 py-3 bg-slate-900 text-white font-bold rounded-xl">确认添加</button></div>
            </form>
        </div>
    </div>
  );
...
const BookingModal = ({ isOpen, onClose, initialDate, initialHour, instrument, onConfirm, isBooking }) => {
  ...
  const getEndDate = () => { if (repeatOption === 0) return "仅今天"; const d = new Date(initialDate); d.setDate(d.getDate() + (repeatOption * 7)); return `直到 ${d.getMonth()+1}月${d.getDate()}日`; };

  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
      <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-slate-800">预约详情</h3><button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X className="w-6 h-6"/></button></div>
        <div className="space-y-4">
          <div className={`${styles.bg} p-4 rounded-xl border-l-4 ${styles.border.replace('border', 'border-l')}`}><div className="text-xs opacity-60 uppercase font-bold mb-1">设备</div><div className={`text-lg font-bold ${styles.text}`}>{instrument?.name}</div>{instrument?.location && <div className="text-xs mt-1 flex items-center gap-1 opacity-80"><MapPin className="w-3 h-3"/> {instrument.location}</div>}</div>
          
          {hasOptions && (
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <div className="text-xs text-slate-400 uppercase font-bold mb-2 flex items-center gap-1"><Plug className="w-3 h-3"/> 选择进样口/附件</div>
          ...
          <div className="flex gap-4"><div className="flex-1 bg-slate-50 p-4 rounded-xl"><div className="text-xs text-slate-400 uppercase font-bold mb-1">日期</div><div className="font-medium text-slate-700">{initialDate}</div></div><div className="flex-1 bg-slate-50 p-4 rounded-xl transition-all"><div className="text-xs text-slate-400 uppercase font-bold mb-1">时段</div><div className={`font-medium ${isFullDay ? 'text-indigo-600 font-bold' : 'text-slate-700'}`}>{isFullDay ? '全天 (24h)' : `${initialHour}:00`}</div></div></div>
          <div onClick={() => setIsFullDay(!isFullDay)} className={`flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-all ${isFullDay ? 'border-indigo-500 bg-indigo-50' : 'border-slate-100 hover:border-slate-200'}`}><div className="flex items-center gap-3"><div className={`p-2 rounded-full ${isFullDay ? 'bg-indigo-200 text-indigo-700' : 'bg-slate-100 text-slate-400'}`}><Sun className="w-5 h-5" /></div><div><div className={`font-bold text-sm ${isFullDay ? 'text-indigo-900' : 'text-slate-600'}`}>全天占用 (24小时)</div><div className="text-xs text-slate-400">预定 00:00 - 23:00</div></div></div>{isFullDay ? <CheckCircle2 className="w-6 h-6 text-indigo-600"/> : <div className="w-6 h-6 rounded-full border-2 border-slate-300"></div>}</div>
          <div className="border-2 border-slate-50 rounded-xl p-4"><div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2 text-slate-700"><Repeat className="w-4 h-4"/><span className="font-bold text-sm">重复频率</span></div><span className="text-xs text-slate-400 font-medium">{getEndDate()}</span></div><div className="grid grid-cols-4 gap-2">{[0, 1, 2, 3].map(opt => (<button key={opt} onClick={() => setRepeatOption(opt)} className={`py-2 rounded-lg text-xs font-bold transition ${repeatOption === opt ? `${styles.darkBg} text-white` : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{opt === 0 ? '单次' : `${opt + 1}周`}</button>))}</div></div>
          <button onClick={() => onConfirm(repeatOption, isFullDay, selectedSubOption)} disabled={isBooking} className={`w-full py-4 text-white font-bold rounded-xl mt-2 flex items-center justify-center gap-2 disabled:opacity-70 ${styles.darkBg}`}>{isBooking ? <Loader2 className="animate-spin w-5 h-5"/> : isFullDay ? "确认全天预约" : "确认预约"}</button>
        </div>
      </div>
    </div>
  );
...
const MemberApp = ({ labName, userName, onLogout }) => {
  ...
  const handleConfirmBooking = async (repeatCount, isFullDay, subOption) => {
    ...
    if (conflicts.length > 0) { alert(`冲突: ${conflicts.join(', ')}`); setIsBookingProcess(false); return; }
    ...
    try {
      ...
      await batch.commit(); 
      await addAuditLog(labName, 'BOOKING', `预约: ${instrument.name} ${subOption ? `[${subOption}]` : ''} (${isFullDay ? '全天' : startHour+':00'})`, userName); 
      setBookingModal({ ...bookingModal, isOpen: false });
    } catch (e) { alert("失败"); } finally { setIsBookingProcess(false); }
  };
  
  const handleDeleteBooking = async () => { if(!bookingToDelete) return; await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'bookings', bookingToDelete.id)); await addAuditLog(labName, 'CANCEL', `取消: ${bookingToDelete.instrumentName}`, userName); setBookingToDelete(null); };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 animate-fade-in">
      ...
      <div className="flex items-center justify-between bg-white p-4 shadow-sm border-b border-slate-100 sticky top-14 z-40 h-16"><button onClick={() => setDate(addDays(date, -1))} className="p-2 hover:bg-slate-100 rounded-full"><ChevronLeft className="w-5 h-5 text-slate-600"/></button><div className="text-center"><span className="font-bold text-slate-800 block">{viewMode === 'day' ? date.toLocaleDateString('zh-CN', { month: 'long', day: 'numeric', weekday: 'short' }) : `${weekDays[0].getMonth()+1}月${weekDays[0].getDate()}日 - ${weekDays[6].getMonth()+1}月${weekDays[6].getDate()}日`}</span></div><button onClick={() => setDate(addDays(date, 1))} className="p-2 hover:bg-slate-100 rounded-full"><ChevronRight className="w-5 h-5 text-slate-600"/></button></div>
      <div className="bg-white border-b border-slate-100 overflow-x-auto no-scrollbar py-2 sticky top-[7.5rem] z-30 h-14 shadow-sm"><div className="flex px-4 gap-2 min-w-max"><button onClick={() => setSelectedTab('overview')} className={`px-4 py-2 rounded-full text-sm font-bold transition flex items-center gap-2 ${selectedTab === 'overview' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-500'}`}><LayoutGrid className="w-4 h-4"/> 总览</button>{instruments.map(inst => { const styles = getColorStyle(inst.color); return (<button key={inst.id} onClick={() => setSelectedTab(inst.id)} className={`px-4 py-2 rounded-full text-sm font-bold transition flex items-center gap-2 ${selectedTab === inst.id ? `${styles.darkBg} text-white shadow-md` : 'bg-slate-100 text-slate-500'}`}><div className={`w-2 h-2 rounded-full ${selectedTab === inst.id ? 'bg-white' : styles.darkBg}`}></div>{inst.name}</button>); })}</div></div>
...
const IdentityScreen = ({ labName, onIdentityVerified }) => {
  ...
  const handleIdentity = async (e) => { e.preventDefault(); if (!name.trim() || !pin.trim()) { setError('请输入名字和个人密码'); return; } setLoading(true); setError('');
    try { const usersRef = collection(db, 'artifacts', appId, 'public', 'data', 'lab_users'); const q = query(usersRef, where('labName', '==', labName), where('userName', '==', name.trim())); const snapshot = await getDocs(q); if (snapshot.empty) { await addDoc(usersRef, { labName, userName: name.trim(), pinCode: pin.trim(), createdAt: serverTimestamp() }); onIdentityVerified(name.trim()); } else { const userData = snapshot.docs[0].data(); if (userData.pinCode === pin.trim()) onIdentityVerified(name.trim()); else setError('密码错误！该名字已被注册。'); }
    } catch (err) { setError('验证失败'); } finally { setLoading(false); }
  };
  return (<div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6 animate-slide-up"><div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8"><button onClick={() => window.location.reload()} className="text-slate-400 text-sm mb-4 flex items-center gap-1"><ChevronLeft className="w-4 h-4"/> 返回</button><div className="flex justify-center mb-6"><div className="bg-indigo-100 p-4 rounded-full"><Fingerprint className="w-10 h-10 text-indigo-600" /></div></div><h1 className="text-2xl font-bold text-center text-slate-800">身份验证</h1><p className="text-center text-slate-500 text-sm mb-6">进入 <span className="font-bold">{labName}</span></p><form onSubmit={handleIdentity} className="space-y-4"><input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="你的名字" className="w-full p-3 rounded-xl border-2 border-slate-100 focus:border-indigo-500 outline-none" /><input type="password" value={pin} onChange={(e) => setPin(e.target.value)} placeholder="个人密码 (PIN)" className="w-full p-3 rounded-xl border-2 border-slate-100 focus:border-indigo-500 outline-none" />{error && <div className="text-red-500 text-xs">{error}</div>}<button type="submit" disabled={loading} className="w-full bg-indigo-600 text-white font-bold py-4 rounded-xl mt-6">{loading ? <Loader2 className="animate-spin mx-auto"/> : "验证并进入"}</button></form></div></div>);
};
